...
{
	return *(int*)(buffer + OFFSET_MAX_HEALTH);
}
...
Vector Entity::getBonePosition( int id)
{
	...
	uint64_t HitboxSet = StudioHdr + mem.Read<uint32_t>(StudioHdr + 0xb4);
	uint32_t GetBone = apex_mem.Read<uint32_t>(HitboxSet + apex_mem.Read<uint32_t>(HitboxSet + 0x8) + (id * 0x2c));
	uint64_t BoneArray = *(uint64_t*)(buffer + OFFSET_BONES);
	matrix3x4_t Matrix = apex_mem.Read<matrix3x4_t>(BoneArray + GetBone * sizeof(matrix3x4_t));
	return Vector(Matrix.m_flMatVal[0][3] + origin.x, Matrix.m_flMatVal[1][3] + origin.y, Matrix.m_flMatVal[2][3] + origin.z);
}*/
/*Vector Entity::getBonePosition_bck(int id)
{
	Vector origin = getPosition();
	...
	int HitboxSet2;
	apex_mem.Read<int>(HitBoxsArray + 0x8, HitboxSet2);

	int Bone;
	apex_mem.Read<int>(HitBoxsArray + HitboxSet2 + (id * 0x2C), Bone);
...
QAngle CalculateBestBoneAim(Entity& from, uintptr_t t, float max_fov)
{
	...
	{
		if (thirdperson) {
			//printf("tirhd preons truee");
		}
		else if (!target.isAlive() || target.isKnocked())
		{
			return QAngle(0, 0, 0);
		}
	}
	...
	float adaptive_dist = LocalPlayerPosition.DistTo(EntityPosition);
	...
	*/
	...
	float new_fov = 33;
	if ((adaptive_dist / 40.0) > 90)new_fov = 5;
	else if ((adaptive_dist / 40.0) > 50)new_fov = 7;
	else if ((adaptive_dist / 40.0) > 20)new_fov = 11;
	else if ((adaptive_dist / 40.0) > 12)new_fov = 18;
	else if ((adaptive_dist / 40.0) > 0.0)new_fov = 30;
	if (fov > new_fov)
	{
		return QAngle(0, 0, 0);
	}
...
{
	//return ((weaponIndex == WeaponIndex::BOW) ? projectileSpeed + pow(lastChargeLevel, 5.4684388195808) : projectileSpeed);
