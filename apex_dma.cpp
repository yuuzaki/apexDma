		...
		while (g_Base!=0 && c_Base!=0)
		{
			...
			else
			{
				for (int i = 0; i < toRead; i++)
				{
					...
					int entity_team = Target.getTeamId();
					if (con_mode) {
						/*if (entity_team % 2 == team_player % 2)
						{
							continue;
						}*/
						if (Target.getMaxHealth() == 80) {
							continue;
						}
					}
					else if(!con_mode) {
	...
	while (aim_t)
	{
		...
		while (g_Base != 0 && c_Base != 0)
		{
			...
			if (aim > 0)
			{
				...
				//no recoil
				LocalPlayer2 = 0;
				apex_mem.Read<uint64_t>(g_Base + OFFSET_LOCAL_ENT, LocalPlayer2);
				if (LocalPlayer2 != 0) {
					LPlayer2 = getEntity(LocalPlayer2);
					punchAG = LPlayer2.GetRecoil();	//change to sway
					punchAG *= 0.60;
					QAngle viewAG = LPlayer2.GetViewAngles();
					QAngle newAG = viewAG + oldPunch - punchAG;
					Math::NormalizeAngles(newAG);

					//printf("%f\n", Math::GetFovNotNormalize(viewAG, newAG));


					if (Math::GetFovNotNormalize(viewAG, newAG) > 90.0) {
						LPlayer2.SetViewAngles(viewAG);
					}
					else {
						LPlayer2.SetViewAngles(newAG); ///go to each empty aim.
						oldPunch = punchAG;
					}
				}
...
static void set_vars(uint64_t add_addr)
{
	...
	uint64_t con_mode_addr = 0;
	client_mem.Read<uint64_t>(add_addr + sizeof(uint64_t) * 18, con_mode_addr);
	...
	while(vars_t)
	{
		...
		while(c_Base!=0 && g_Base!=0)
		{
			...
			client_mem.Read<bool>(con_mode_addr, con_mode);
			if (t_mode != con_mode) {
				printf("mode_change");
...
int main(int argc, char *argv[])
{
	...
	const char* cl_proc = "Reapog.exe";
	const char* ap_proc = "r5apex.exe";
	//const char* ap_proc = "EasyAntiCheat_launcher.exe";

	//Client "add" offset
	uint64_t add_off = 0x2d8b0;
